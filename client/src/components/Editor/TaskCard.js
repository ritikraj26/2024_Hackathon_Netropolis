...
const TaskCard = ({ task, minus, showSelected, notEditable }) => {
  ...
  return (
    <Banner
      className={
        (notEditable ? " " : showSelected === task.selected ? " " : " hidden") +
        " mb-2 border-primary-600 relative "
      }
      onClick={handleTaskClick}
    >
      <div className="relative left-1/2 -translate-x-1/2 hover:bg-gray-100 flex w-[calc(100%-2rem)] flex-row justify-start rounded-lg border border-gray-300 bg-white p-4 shadow-sm dark:border-gray-600 dark:bg-gray-700 md:flex-row lg:max-w-7xl">
        ...
        <div className="mb-3 mx-4 flex flex-col items-start md:mb-0 md:flex-row md:items-center">
          <p className="text-start te flex items-center text-sm font-normal text-gray-700 dark:text-gray-400 ">
            {task.name}
          </p>
        </div>
        {!notEditable && (
